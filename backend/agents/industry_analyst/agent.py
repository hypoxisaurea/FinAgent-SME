...
class IndustryAnalystAgent(Agent):
    ...
    async def run(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        industry_avg, industry_avg_run = execute_tool_step(
            ...
            fallback_factory=lambda: _default_industry_summary(ksic_code, target_year),
            validate_dict=True,
        )
        ...
        logger.info(
            (
                "industry_analysis_finished request_id=%s company_name=%s "
                "corp_code=%s ksic_code=%s tool_error_count=%s"
            ),
            request_id,
            company_name,
            corp_code,
