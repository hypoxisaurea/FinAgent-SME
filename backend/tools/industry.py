...
# ===========================================================================
# KNOWN ISSUE: DART induty_code는 3/4/5자리 등 길이가 일정하지 않은데
# 현재 [:4] 슬라이싱으로만 매칭한다. 3자리 코드(예: 삼성전자 "264")는
# 4자리 키와 영원히 안 맞아서, 이 경로는 사실상 회사명 키워드 매칭이
# 실질적 메인 경로로 동작 중. 기존 14개 업종 매핑도 동일한 한계 있음.
# 데모 이후 정리 필요 (길이별 매칭 또는 startswith 양방향 체크로 개선).
_MANUFACTURING_INDUTY_CODE_TO_SUB_SECTOR: dict[str, str] = {
    ...
    "3040": "방산",
    "2591": "방산",
    # 자동차업 (KSIC 2910: 자동차 제조, 2920: 차체·특장차)
    "2910": "자동차",
    "2920": "자동차",
    # 자동차부품업 (KSIC 2930: 자동차 부품)
    "2930": "자동차부품",
    # 전선업 (KSIC 2731: 절연선·케이블, 2732: 광섬유케이블)
    "2731": "전선",
    "2732": "전선",
    # 화섬업 (KSIC 2040: 화학섬유)
    "2040": "화섬",
    # 제지업 (KSIC 1711: 펄프, 1712: 골판지 원지, 1719: 기타 종이)
    "1711": "제지",
    "1712": "제지",
    "1719": "제지",
    # 시멘트업 (KSIC 2391: 석회·석고·시멘트)
    "2391": "시멘트",
    # 제약업 (KSIC C2110: 기초의약물질·생물학적 제제, C2120: 완제의약품)
    # "2110" / "2120" : 표준 5자리 코드(21101, 21201 등) [:4] 결과
    # "212"           : 유한양행·고려제약의 실제 DART 3자리 induty_code ([:4] = "212")
    # "2121"          : 한미약품 실제 DART "21212"의 [:4] 결과
    "2110": "제약업",
    "2120": "제약업",
    "212":  "제약업",
    "2121": "제약업",
    # 음식료업 — 식료품(10xx) 및 음료(11xx) 주요 코드
    "1010": "식음료",  # 도축·육류 가공
    "1020": "식음료",  # 수산물 가공
    "1030": "식음료",  # 과실·채소 가공
    "1040": "식음료",  # 동물성·식물성 유지
...
_MANUFACTURING_COMPANY_NAME_KEYWORDS: tuple[tuple[str, str], ...] = (
    ...
    ("beverage", "식음료"),
    # 제약업
    ("바이오제약",      "제약업"),   # 바이오제약이 제약보다 먼저 — 더 구체적
    ("원료의약품",      "제약업"),
    ("제약",            "제약업"),
    ("의약품",          "제약업"),
    ("pharmaceutical",  "제약업"),
    ...
    if company_name:
        ...
        for keyword, sub_sector in _MANUFACTURING_COMPANY_NAME_KEYWORDS:
            if keyword.lower() in lower_name:
                return sub_sector
    # 코드·명칭 매칭 전부 실패 → 제조업 기본 폴백 (Corporate(제조) 방법론 적용)
    return "corporate_제조"
